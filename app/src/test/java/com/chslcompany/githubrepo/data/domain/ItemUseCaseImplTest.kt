...
class ItemUseCaseImplTest : TestCase() {
    private val testCoroutineDispatcher = TestCoroutineDispatcher()
    private val repository = mock(KotlinRepositoriesImpl::class.java)

    @Test
    fun `test response useCase`() = testCoroutineDispatcher.runBlockingTest {
        val useCase = ItemUseCaseImpl(repository)
        val mockUseCaseResponse = mockResponse()

        testCoroutineDispatcher.pauseDispatcher()

        doReturn(mockUseCaseResponse).`when`(repository).getKotlinRepositories(
            anyString(),
            anyString(),
            anyInt()
        )
        ...
        useCase.getKotlinRepository(anyString(), anyString(), anyInt()).let {
            Truth.assertThat(it).isEqualTo(mock())
        }
